...
void Tlc5940::init(uint16_t initialValue)
{
    ...
    // HSPI_MISO is -1 (unused) on ESP32-S3 -- do not configure.

    // SPI Setup
    // On ESP32-S3 the HSPI peripheral has no default pin map; we must pass
    // the pins explicitly. MISO=-1 tells the driver to skip attaching it.
    this->spi = new SPIClass(HSPI);
    this->spi->begin(HSPI_SCLK, HSPI_MISO, HSPI_MOSI, HSPI_CS);
    this->spi->beginTransaction(SPISettings(8000000, MSBFIRST, SPI_MODE3));

    // Greyscale clock via LEDC
